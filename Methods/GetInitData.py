...
def getInitData(user_id):
    ...
    for i in range(0, row_number):
        ...
        temp["fromUsername"] = row["from_username"]
        temp["toId"] = row["to_id"]
        temp["toUsername"] = row["to_username"]
        notificationData.append(temp)

    #组装gameTableData
    row_number=cursor.execute("SELECT a.table_id,a.left_player_id,c.username AS left_username,b.avatar AS left_avatar,a.right_player_id,e.username AS right_username,d.avatar AS right_avatar,a.game_state FROM game_table_info AS a LEFT join user_info AS b ON a.left_player_id=b.user_id LEFT JOIN  user AS c ON b.user_id=c.user_id LEFT JOIN user_info AS d ON a.right_player_id=d.user_id LEFT JOIN user AS e ON e.user_id=d.user_id")
    for i in range(0, row_number):
        row = cursor.fetchone()
        temp={}
        temp["tableId"]=row["table_id"]
        temp["leftPlayerId"] = row["left_player_id"]
        temp["leftUsername"] = row["left_username"]
        temp["leftAvatar"] = row["left_avatar"]
        temp["rightPlayerId"] = row["right_player_id"]
        temp["rightUsername"] = row["right_username"]
        temp["rightAvatar"] = row["right_avatar"]
        temp["gameState"] = row["game_state"]
