...
class PutChessHandler(tornado.web.RequestHandler):
    def post(self):
        ...
        # TODO(MICHAEL)后面需要添加判断输赢的函数，然若有输赢则做相应处理
        winFlag=False
        drawFlag=False
        if judgeWin(pointState,{"chessType":chessType,"chessPoint":[int(x),int(y)]}):
            ...
            cursor.execute("UPDATE user_info SET game_time=game_time+1 WHERE user_id=%s OR user_id=%s",(userId,anotherPlayerId))
        ...
        if len(roomCache["stepRecord"])==196:
            ...
            roomCache["playerState"][anotherPlayerId]["myTurn"] = False
            roomCache["totalGameTime"] += 1
            cursor.execute("UPDATE user_info SET game_time=game_time+1,draw_time=draw_time+1 WHERE user_id=%s OR user_id=%s",
